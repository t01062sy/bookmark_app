...
- **API 通信未実装**: バックエンド完成後に統合予定
- **認証機能未実装**: WebAuthn パスキー（バックエンド依存）
...
#### 🎯 Phase 1B - Week 2: 基本 API 実装
- [ ] **Edge Functions 作成**
  - `POST /v1/bookmarks` (基本保存)
  - `GET /v1/bookmarks` (一覧取得・検索)
  - `POST /v1/auth/webauthn/*` (認証エンドポイント)
- [ ] **メタデータ取得機能**
  - OGP/oEmbed パーサー実装
...
- **Phase 1B (バックエンド)** 🚧 **次回開始** - Supabase セットアップから

### 🚀 最優先アクション
1. **Supabase プロジェクト作成・DB スキーマ構築** (2-3 日)
2. **最初の API エンドポイント実装** (2-3 日) 
3. **iOS アプリ API 連携** (1-2 日)

### 📊 開発期間見積もり
- **Phase 1B (バックエンド基盤)**: 2-3 週間
